...
void CustomizationApp::renderButtons(int &mouseX, int &mouseY) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    SDL_Surface *surface =
        TTF_RenderText_Solid(_font, tempstr[i].c_str(), {0, 0, 0, 255});

    SDL_Texture *texture =
        SDL_CreateTextureFromSurface(_editorWindow->getRenderer(), surface);
    SDL_FreeSurface(surface);

    SDL_Rect textDestRect = {textX, textY, textWidth, textHeight};
    SDL_RenderCopy(_editorWindow->getRenderer(), texture, nullptr,
                   &textDestRect);
...
bool CustomizationApp::isMouseOverBtns(int &mouseX, int &mouseY,
                                       SDL_Rect &temp) {
  return mouseX >= temp.x && mouseX <= (temp.x + temp.w) && mouseY >= temp.y &&
         mouseY <= (temp.y + temp.h);
}
        ...
        case 0:
          if (isBackgroundSelected) {
            Uint8 red = static_cast<Uint8>(
                std::max(0, std::min(255, sliderRed->getValue())));
            Uint8 green = static_cast<Uint8>(
                std::max(0, std::min(255, sliderGreen->getValue())));
            Uint8 blue = static_cast<Uint8>(
                std::max(0, std::min(255, sliderBlue->getValue())));
