	...
	: _title (title), _file_name ("unnamed"), _height (height), _width (width)
{
	...
	_window = SDL_CreateWindow (temp.c_str (), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, _width, _height,
...
int EditorWindow::getWindowWidth () const
{
	int width, height;
...
int EditorWindow::getWindowHeight () const
{
	int width, height;
	SDL_GetWindowSize (_window, &width, &height);
	return height;
...
SDL_Renderer *EditorWindow::getRenderer () const
{
	return _renderer;
}

SDL_Window *EditorWindow::getWindow () const
{
	return _window;
}
// TTF_Font *
