...
MenuBar::MenuBar (EditorWindow *w, InputEditor *ie)
	: _windowEditor (w), _inputEditor (ie), _renderer (_windowEditor->getRenderer ())
{
	int tempx = 0;
	...
	for (const auto &el: tempStr)
	{
		...
		SDL_Surface *tempSurf = TTF_RenderText_Solid (_inputEditor->getFont (), tempStr.c_str (), _fontColor);
	...
	for (const auto &el: tempStr)
	{
		...
		SDL_Rect rect = {tempx, tempy, maxWidth, textHeight}; // Ustaw szerokość na maksymalną

		selFile.push_back ({el, tempx, tempy, maxWidth, textHeight, col, false, rect});
		tempy += textHeight - 1;
	}
	// Edit Undo Paste
	...
	for (const auto &el: tempStr)
	{
		...
		selEdit.push_back ({el, tempx, tempy, maxWidth, textHeight, col, false, rect});
		tempy += textHeight - 1;
	}
	// optiosn
	...
	for (const auto &el: tempStr)
	{
		...
		selOptions.push_back ({el, tempx, tempy, maxWidth, textHeight, col, false, rect});
		tempy += textHeight - 1;
	}
	for (auto &el: selOps)
	{
...
MenuBar::~MenuBar ()
{
	std::cout << "MenuBar object has been destroyed\n";
}
...
void MenuBar::textsRender ()
{
	...
	for (auto &button: selOps)
	{
		...
		SDL_SetRenderDrawColor (_renderer, button.clr.r, button.clr.g, button.clr.b, button.clr.a);
		SDL_RenderFillRect (_renderer, &button.rect);
		SDL_SetRenderDrawColor (_renderer, 0, 0, 0, 255);
		SDL_RenderDrawRect (_renderer, &button.rect);

		int textWidth, textHeight;
		TTF_SizeText (_inputEditor->getFont (), button.name.c_str (), &textWidth, &textHeight);
		...
		SDL_Surface *surface = TTF_RenderText_Solid (_inputEditor->getFont (), button.name.c_str (), _fontColor);
...
void MenuBar::renderOpsOption (std::vector<selectOption> &vecOps)
{
	for (auto &el: vecOps)
	{
		...
		SDL_Surface *tempSurf = TTF_RenderText_Solid (font, el.name.c_str (), _fontColor);
		SDL_Texture *tempTexture = SDL_CreateTextureFromSurface (_renderer, tempSurf);

		SDL_Rect textDestRect = {el.rect.x + (el.rect.w - tempSurf->w) / 2, el.rect.y + (el.rect.h - tempSurf->h) / 2,
	...
	if (e.type == SDL_MOUSEMOTION)
	{
		...
		if (isMouseOver (mouseX, mouseY, selOps[ 0 ]))
		{
			...
			fileOps = true;
				  ...
				  && (mouseX >= selOps[ 0 ].x && mouseX <= selOps[ 0 ].x + selOps[ 0 ].w && mouseY >= selOps[ 0 ].y
					  && mouseY <= selFile[ selFile.size () - 1 ].y + selFile[ selFile.size () - 1 ].h)))
		{
		...
		if (isMouseOver (mouseX, mouseY, selOps[ 1 ]))
		{
			...
			editOps = true;
				  ...
				  && (mouseX >= selOps[ 1 ].x && mouseX <= selOps[ 1 ].x + selOps[ 1 ].w && mouseY >= selOps[ 1 ].y
					  && mouseY <= selEdit[ selEdit.size () - 1 ].y + selEdit[ selEdit.size () - 1 ].h)))
		{
		...
		if (isMouseOver (mouseX, mouseY, selOps[ 2 ]))
		{
			...
			opsOps = true;
				  ...
				  && (mouseX >= selOps[ 2 ].x && mouseX <= selOps[ 2 ].x + selOps[ 2 ].w && mouseY >= selOps[ 2 ].y
					  && mouseY <= selOptions[ selOptions.size () - 1 ].y + selOptions[ selOptions.size () - 1 ].h)))
		{
	...
	if (e.type == SDL_MOUSEBUTTONDOWN)
	{
		...
		if (fileOps)
		{
			for (auto &el: selFile)
			{
				...
				{
					int btn;
					this->PopBackWindow (btn, "Creating new file",
										 "Are you really want to create new file and leave current?", "Yes", "No");
					if (btn == 0)
					{
				...
				if (isMouseOver (mouseX, mouseY, el) && el.name == "Open")
				{
					...
					_filemanager = new FileManager (_savingWindow, _inputEditor, _windowEditor->getPath (), "Open");
					bool sQuit = false;
					SDL_Event sEvent;
					while (! sQuit)
					{
						while (SDL_PollEvent (&sEvent) != 0)
						{
							...
							if (sEvent.type == SDL_QUIT)
							{
								sQuit = true;
									 ...
									 && sEvent.window.windowID == SDL_GetWindowID (_savingWindow->getWindow ()))
							{
								sQuit = true;
							}
				...
				{
					SaveManager savingProc (_windowEditor->getFileName (), _inputEditor->getTextContent (),
				...
				if (isMouseOver (mouseX, mouseY, el) && el.name == "Save as")
				{
					...
					_filemanager = new FileManager (_savingWindow, _inputEditor, _windowEditor->getPath ());
					bool sQuit = false;
					SDL_Event sEvent;
					while (! sQuit)
					{
						...
						while (SDL_PollEvent (&sEvent) != 0)
						{
							...
							if (sEvent.type == SDL_QUIT)
							{
								sQuit = true;
									 ...
									 && sEvent.window.windowID == SDL_GetWindowID (_customWindow->getWindow ()))
							{
								sQuit = true;
							}
...
void MenuBar::PopBackWindow (int &btn, std::string title, std::string message, std::string yes, std::string no)
{
	...
	SDL_MessageBoxButtonData buttons[] = {{SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT, 0, yes.c_str ()},
	...
	SDL_MessageBoxColorScheme mescolt
		= {messBackground, messText, messButtonBorder, messButtonBackground, messButtonSelected};
...
void MenuBar::PopBackWindow (int &btn, std::string title, std::string message, std::string yes)
{
	SDL_MessageBoxData messageboxdata;
	messageboxdata.flags = SDL_MESSAGEBOX_WARNING;
	messageboxdata.window = _windowEditor->getWindow ();
