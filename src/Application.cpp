...
int Application::run ()
{
	...
	while (! quit)
	{
		...
		while (SDL_PollEvent (&event) != 0)
		{
			...
			if (event.type == SDL_QUIT)
			{
				...
				else
				{
					...
					else if (btn == 1)
					{
						...
						_filemanager = new FileManager (_savingWindow, _textEditor, _window->getPath ());
						bool sQuit = false;
						SDL_Event sEvent;
						while (! sQuit)
						{
			...
			SDL_SetRenderDrawColor (_window->getRenderer (), _backgroundColor.r, _backgroundColor.g, _backgroundColor.b,
									_backgroundColor.a);
...
void Application::popUpWindow (int &btn)
{
	...
	SDL_MessageBoxButtonData buttons[]
		= {{SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT, 0, "Yes"}, {SDL_MESSAGEBOX_BUTTON_RETURNKEY_DEFAULT, 1, "No"}};
