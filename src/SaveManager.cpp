...
SaveManager::SaveManager (std::string fileName, std::string content, std::filesystem::path path)
...
void SaveManager::openFile (bool &q, SDL_Window *w, InputEditor *ie)
{
...
bool SaveManager::isFileExist () const
{
...
void SaveManager::PopUp (SDL_Window *w, std::string title, std::string mess)
{
	int btn;
	...
	SDL_MessageBoxButtonData buttons[] = {{SDL_MESSAGEBOX_BUTTON_ESCAPEKEY_DEFAULT, 0, "Ok"}};
...
void SaveManager::saveFile (bool &q, SDL_Window *w)
{
	if (! this->isFileExist ())
	{
		...
		savingFile << _content;
		savingFile.close ();
		std::cout << "New file " << _file_name << " has been created in " << _new_file_path << "\n";
...
void SaveManager::saveFile (SDL_Window *w)
{
	if (this->isFileExist ())
	{
		std::ofstream savingFile{_file_name};
