...
FileManager::FileManager (EditorWindow *w, InputEditor *iw, std::filesystem::path path)
	: _file_name ("New_file.txt"), _path (path), _windowSaving (w), _savingInput (iw),
...

{
	...
	{
		btnGroup.push_back (
			{btnXY[ i ][ 0 ], btnXY[ i ][ 1 ], btnWH[ i ][ 0 ], btnWH[ i ][ 1 ], btnNames[ i ], {255, 255, 255, 255}});
	}
}
FileManager::FileManager (EditorWindow *w, InputEditor *iw, std::filesystem::path path, std::string open)
...
void FileManager::loadFont (const std::string &fontPath, int fontSize)
{
...
void FileManager::renderCurrentPath ()
{
	int textWidth, textHeight;
	SDL_Surface *tempSurface = TTF_RenderText_Solid (_font, _path.string ().c_str (), {255, 255, 255, 255});
...
void FileManager::renderFiles ()
{
	...
	for (const auto &entry: std::filesystem::directory_iterator (_path))
	{
		...
		SDL_Surface *tempSurface = TTF_RenderText_Solid (_font, temp.c_str (), textColor);
		SDL_Texture *tempTexture = SDL_CreateTextureFromSurface (_renderer, tempSurface);
		SDL_QueryTexture (tempTexture, NULL, NULL, &textWidth, &textHeight);
...
void FileManager::renderNewFile ()
{
	int windowWidth, windowHeight;
	SDL_GetWindowSize (_windowSaving->getWindow (), &windowWidth, &windowHeight);

	int textWidth, textHeight;
	SDL_Surface *tempSurface = TTF_RenderText_Solid (_font, _file_name.c_str (), {255, 255, 255, 255});
	SDL_Texture *tempTexture = SDL_CreateTextureFromSurface (_renderer, tempSurface);
	SDL_QueryTexture (tempTexture, NULL, NULL, &textWidth, &textHeight);
...
void FileManager::renderBlankSpaces ()
{
	int windowWidth, windowHeight;
	SDL_GetWindowSize (_windowSaving->getWindow (), &windowWidth, &windowHeight);
...
void FileManager::btnRender (MyButton butt)
{
	int windowWidth, windowHeight;
	SDL_GetWindowSize (_windowSaving->getWindow (), &windowWidth, &windowHeight);
	...
	SDL_SetRenderDrawColor (_renderer, butt.buttonColor.r, butt.buttonColor.g, butt.buttonColor.b, butt.buttonColor.a);

	SDL_Rect buttonFillRect{butt.btnX + 1, butt.btnY + 1, butt.btnW - 2, butt.btnH - 2};
	SDL_RenderFillRect (_renderer, &buttonFillRect);

	SDL_Surface *textSurface = TTF_RenderText_Solid (_font, butt.name.c_str (), {0, 0, 0, 255});
...
bool FileManager::IsMouseOver (int mouseX, int mouseY, MyButton btn) const
{
	return mouseX >= btn.btnX && mouseX <= btn.btnX + btn.btnW && mouseY >= btn.btnY && mouseY <= btn.btnY + btn.btnH;
}

void FileManager::changeButtonColor (MyButton &btn, SDL_Color col)
{
	btn.buttonColor = col;
}
void FileManager::handleEventMouse (SDL_Event &e, bool &q)
{
	...
	{
		int count = std::distance (std::filesystem::directory_iterator (_path), std::filesystem::directory_iterator{});
...
void FileManager::handleButtonClick (const MyButton &btn, bool &q)
{
	// next
	if (btn.name == ">")
	{
	...
	{
		SaveManager savingProc (_file_name, _savingInput->getTextContent (), _path);
		savingProc.saveFile (q, _windowSaving->getWindow ());
		_windowSaving->changeNewFileName (_file_name);
	}
}

bool FileManager::IsMouseOverFolderFile (const std::filesystem::directory_entry &entry, int index) const
{
	int mouseX, mouseY;
	...
	return mouseX >= 10 && mouseX <= 10 + textWidth && mouseY >= posY && mouseY <= posY + textHeight;
}

void FileManager::handleEventKeyboard (SDL_Event &e, bool &q)
{
	if (e.type == SDL_QUIT)
	{
		q = true;
...
void FileManager::handleKeyboard (SDL_Event &e)
{
	if ((e.key.keysym.sym >= SDLK_SPACE && e.key.keysym.sym <= SDLK_z))
	{
		...
		_file_name.insert (static_cast<int> (currentCursorPosition) + 1, 1, pressedChar);
		currentCursorPosition += 1;
	}
}

void FileManager::renderCursor ()
{
	int textWidth, textHeight;
	int windowHeight, windowWidth;
	SDL_GetWindowSize (_windowSaving->getWindow (), &windowWidth, &windowHeight);
	SDL_Surface *indexSurface = TTF_RenderText_Solid (_font, "|", {255, 255, 255, 255});
...
void FileManager::handleBackspace ()
{
	if (! _file_name.empty ())
	{
...
void FileManager::handleEventMouseOpen (SDL_Event &e, bool &q)
{
	if (e.type == SDL_MOUSEMOTION)
	{
		int mouseX = e.motion.x;
...
void FileManager::handleButtonClickOpen (const MyButton &btn, bool &q)
{
	// next
	if (btn.name == ">")
	{
	...
	else if (btn.name == "Open")
	{
		...
		savingProc.openFile (q, _windowSaving->getWindow (), _editorInput);
		_windowSaving->changeNewFileName (_file_name);
		_editorInput->resetHistory ();
	}
}
